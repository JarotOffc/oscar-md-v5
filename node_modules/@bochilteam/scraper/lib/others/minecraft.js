...
function statusJava(ip, port = 25565, opts = { timeout: 5 * 1000 }) {
    return new Promise(async (resolve, reject) => {
        var _a;
        setTimeout(() => reject(new utils_1.ScraperError("timeout")), opts.timeout);
...
class TCPsocket extends events_1.default {
    ...
    connect(opts) {
        ...
        this.socket.on('error', () => {
            this.reject(new utils_1.ScraperError('Connection error'));
            this.emit('error');
        });
        this.socket.on('timeout', () => {
            this.reject(new utils_1.ScraperError('Connection timeout'));
            this.emit('timeout');
        });
    }
    write(data) {
