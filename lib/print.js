...
`.trim(),
  ...
  switch(m.mtype) {
      ...
      case 'audioMessage':
        s = m.msg.seconds
        detik = `${Math.floor(s/ 60).toString().padStart(2, 0)}:${(s % 60).toString().padStart(2, 0)}`
        if(m.msg.ptt){
          console.log(`🎤 Voice ${detik}\n`)
        } else {
          console.log(`🎵 Audio ${detik}\n`)
        }
        break
      case 'stickerMessage':
        if(m.msg.isAnimated) {
          console.log(`StickerGif\n`)
        } else {
        	console.log(`Sticker\n`) 
        }
        break
